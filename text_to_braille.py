...
def is_all_caps(s):
    ...
    False
    '''
    #check if the input is greater than one character
    if(len(s)>1):
        #check if every charater in s is capitalized
        for char in s:
            #if not return False
            if(not is_capitalized(char)):
                return False
        #if all are capitalized, return True
        return True
...
def word_to_braille(word):
    ...
    .o .o oo o. .. oo oo
    '''
    #check if the input is all capitalized or not
    if(is_all_caps(word)):
        #add two CAPITAL symbol before it
        ostring=CAPITAL+'\n\n'+CAPITAL+'\n\n'
        #convert the word letter by letter
        for char in word:
            #add two new lines between each word
            ostring+=(convert_letter(char)+'\n\n')
        return ostring
    #if the input is not all capitalized
    else:
        #create a new variable to store the word ostring
        ostring=''
        #convert the word letter by letter 
        for char in word:
            #if the letter is a digit, add NUMBER symbol before it
            if(is_digit(char)):
                ostring+=NUMBER+'\n\n'
            #if the letter is a capitalized letter, add CAPITAL symbol before it 
            if(is_capitalized(char)):
                ostring+=CAPITAL+'\n\n'
            #convert the letter to ostring, and add it to the variable ostring
            ostring+=(char_to_braille(char)+'\n\n')    
        return ostring
...
def new_filename(fname, addition):
    ...
    '''
    #find the dot first to sperate the filename and extension
    index=fname.find('.')
    #create a variable to store the value of filename
    s=fname[:index]
    #create a variable to store the value of extension
    extension=fname[index:]
    #add the _ and  addition part to the final filename
    result=s+'_'+addition+extension
